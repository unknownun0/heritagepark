...
import { getPostFromDb } from '@/lib/content'
...
function contentToHtml(content: string | string[]): string {
...
export default async function BlogPostSection({ slug }: Props) {
  ...
  const staticPost = dbPost ? null : getPostBySlug(slug)
  ...
  if (dbPost) {
    post = {
      ...
      topicId: dbPost.topic_id,
      topic: dbPost.topic,
      lang: dbPost.lang,
      image: dbPost.image || '/images/blog-planning.jpg',
      readTime: dbPost.read_time,
      content: dbPost.content ? dbPost.content.split('\n\n').filter(Boolean) : [],
      relatedSlugs: [],
    ...
    post = staticPost
    relatedPosts = getRelatedPosts(staticPost)
  }
    ...
    <>
      <section className="bg-cream">
        <div className="max-w-4xl mx-auto px-4 py-12">
          ...
          <div className="aspect-video bg-moss/10 rounded-lg flex items-center justify-center mb-8" style={post.image ? { backgroundImage: `url(${post.image})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}>
            {!post.image && (
          ...
          <div className="bg-primary rounded-lg p-6 md:p-8 my-10 text-center">
            ...
            <a href="#" className="inline-block bg-[#e2af43] text-primary font-semibold px-8 py-3 rounded text-sm hover:bg-[#e2af43]/90 transition-colors">Download Free Guide</a>
          </div>
